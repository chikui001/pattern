package com.chiao.pattern.widgets.snackbar;
...
public final class MySnackbar {
	...
	public static abstract class Callback {
		...
		@IntDef({ DISMISS_EVENT_SWIPE, DISMISS_EVENT_ACTION, DISMISS_EVENT_TIMEOUT, DISMISS_EVENT_MANUAL,
		...
		public void onDismissed(MySnackbar snackbar, @DismissEvent int event) {
	...
	public static MySnackbar make(@NonNull View view, @NonNull CharSequence text, @Duration int duration) {
		MySnackbar snackbar = new MySnackbar(findSuitableParent(view));
	...
	@Deprecated
	public MySnackbar addIcon(int resource_id, int size) {
		final TextView tv = mView.getMessageView();

		tv.setCompoundDrawablesWithIntrinsicBounds(new BitmapDrawable(
				Bitmap.createScaledBitmap(((BitmapDrawable) (mContext.getResources().getDrawable(resource_id))).getBitmap(), size,
										  size, true)), null, null, null);

		return this;
	}

	public MySnackbar setIconPadding(int padding) {
		final TextView tv = mView.getMessageView();
		tv.setCompoundDrawablePadding(padding);
		return this;
	}

	public MySnackbar setIconLeft(@DrawableRes int drawableRes, float sizeDp) {
		final TextView tv = mView.getMessageView();
	...
	public MySnackbar setIconRight(@DrawableRes int drawableRes, float sizeDp) {
		final TextView tv = mView.getMessageView();
		Drawable drawable = ContextCompat.getDrawable(mContext, drawableRes);
		if (drawable != null) {
			drawable = fitDrawable(drawable, (int) convertDpToPixel(sizeDp, mContext));
	 ...
	 * @return this MySnackbar
	 */
	public MySnackbar setMaxWidth(int maxWidth) {
	...
	@NonNull
	public MySnackbar setActionOnClickListener(final View.OnClickListener listener) {

		final RelativeLayout actionLayout = mView.getActionLayout();
		actionLayout.setVisibility(View.VISIBLE);

		final TextView messageView = mView.getMessageView();
		RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) messageView.getLayoutParams();
		int margin = mContext.getResources().getDimensionPixelSize(R.dimen.snack_left_right_space);
	...
	public MySnackbar setMessageOnClickListener(final View.OnClickListener listener) {
		final TextView messageView = mView.getMessageView();
	...
	private void animateViewIn() {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			...
			Animation anim = android.view.animation.AnimationUtils.loadAnimation(mView.getContext(), R.anim.top_in);
	...
	private void animateViewOut(final int event) {
		if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.ICE_CREAM_SANDWICH) {
			ViewCompat.animate(mView).translationY(-mView.getHeight()).setInterpolator(new FastOutSlowInInterpolator())
					  .setDuration(ANIMATION_DURATION).setListener(new ViewPropertyAnimatorListenerAdapter() {
	...
	public static class SnackbarLayout extends RelativeLayout {
		...
		protected void onFinishInflate() {
			...
			mActionLayout = (RelativeLayout) findViewById(R.id.snackbar_action_ry);
		...
		RelativeLayout getActionLayout() {
